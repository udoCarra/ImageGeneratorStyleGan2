...
class ImageGenerator:
    ...
    def project_with_progressive_growing(self, initial_resolution=64, final_resolution=1024, nb_steps_per_resolution=5000, stop_threshold=1e-5, reg_weight=0.0005):
        """
        Projette une image dans l'espace latent du modèle StyleGAN3 en utilisant la méthode de Progressive Growing basée sur l'interpolation des latents.
        ...
        for resolution in resolutions:
            ...
            # Si nous avons une résolution précédente, nous interpolons les latents
            ...
            for step in range(nb_steps_per_resolution):
                ...
                # Interpolation progressive des latents
                alpha = step / nb_steps_per_resolution
                interpolated_w_plus = self._lerp(old_w_plus, w_plus, alpha)

                # Générer une image à partir du vecteur latent interpolé
                img_gen = self.model.synthesis(interpolated_w_plus, noise_mode='const', force_fp32=True)
                ...
                total_loss = 0.2 * perceptual_loss + 0.8 * mse_loss + reg_weight * torch.norm(interpolated_w_plus, p=2)
